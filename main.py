...
pygame_icon = pygame.image.load('bird.png')
...
class Game:
    ...
    def game_over(self):
        if self.is_collide():
            ...
            self.display_text("Game Over!", (255, 255, 255), 450, 300, 84, "Times New Roman", bold=True)
            self.display_text("Press Enter To Play Again", (0, 0, 0), 400, 600,50, "Times New Roman", bold=True)
            self.pipe_velocity = 0
            self.flap = 0
    ...
    def main_game(self):
        while self.game_on:
            ...
            self.flapping()
            self.game_over()
            self.display_text(str(self.score), (255, 255, 255), 600, 50, 68, "Arial", bold=True)
